...
class SparkGraphAsPipe[S,E](name: String) extends BulkPipe[S,E] {
  ...
  def bulkProcess(): BulkPipeData[E] = {
    ...
    val out = bs.elementType match {
      case BulkDataType.VERTEX_DATA => bs.stateGraph.mapVertices( (x,y) => (y._1, GremlinVertex.addAsColumn( y._2, tname, x, y._1)) );
      case BulkDataType.VERTEX_PROP_DATA => bs.stateGraph.mapVertices( (x,y) => (y._1, GremlinVertex.addAsColumn( y._2, tname, x, y._1.getProperty(tkey) )) )
      case _ => throw new RuntimeException("Don't know what to do here")
    }

    if (bs.asColumns != null ) {
      return new SparkGraphBulkData[E](bs.graphData, out, bs.asColumns ++ Array((name, bs.elementType)), bs.elementType, bs.extractKey) {
        def currentRDD(): RDD[E] = bs.currentRDD()
      }
    }
    return new SparkGraphBulkData[E](bs.graphData, out, Array((name, bs.elementType)), bs.elementType, bs.extractKey) {
      def currentRDD(): RDD[E] = bs.currentRDD()
    }
